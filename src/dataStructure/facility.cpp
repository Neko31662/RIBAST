...
double productValueList[FACILITY_TYPE_COUNT + 1][6];
...
bool isUniqueFacility(int facilityType) {
    return facilityType == CONTROL || facilityType == MEETING || facilityType == OFFICE ||
...
void loadValueList() {
    ...
    for (int i = 1; i <= FACILITY_TYPE_COUNT; i++) {
        for (int j = 1; j <= 5; j++) {
    ...
    for (int j = 1; j <= 3; j++) {
        ...
        productValueList[MEETING][j] = productValue_perClue;
...
double getProductValue(int facilityType, int level) {
    ...
    if (facilityType < 1 || facilityType > FACILITY_TYPE_COUNT || level < 1 || level > 5) {
        throw invalid_argument("getProductValue函数：facilityType或level参数错误");
    }
    return productValueList[facilityType][level];
}

string getFacilityTypeName(int facilityType) {
    switch (facilityType) {
    case CONTROL:
        return "控制中枢";
    case MFG_GOLD:
